...
#apriori call
@st.cache
def apriori_algo(df, min_support, max_len):
    return apriori(items_df, min_support = min_support, max_len = max_len, use_colnames=True)
#FPgrowth calls
@st.cache
...
algo_dict = {'FPgrowth':fpgrowth_algo,'Apriori':apriori_algo}
...
with col2:
    if option == 'support':
        st.select_slider('min_thresh',options=sup_opts, value=np.min(sup_opts))
    elif option == 'lift':
        st.select_slider('min_thresh',options=lift_opts, value=np.min(lift_opts))
        pass
